...
import engine
import prices as px
import context as mctx
from config import CHAT_ID, WAT
...
async def send_morning_briefing(context: ContextTypes.DEFAULT_TYPE):
    ...
    now_wat = _now_wat().strftime('%H:%M')
    lines = [
        "☀️ *Daily Briefing*",
        "━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Session: `{engine.get_session()}` | 🕐 `{now_wat} WAT`",
    ...
    if w:
        ach = db.update_weekly_achieved()
        tgt = float(w.get('r_target') or 0)
    ...
    if mg:
        ach = float(mg.get('r_achieved') or 0)
        tgt = float(mg.get('r_target') or 0)
        pct = int((ach / tgt) * 100) if tgt else 0
        bar = '█' * max(0, min(10, pct // 10)) + '░' * (10 - max(0, min(10, pct // 10)))
...
async def send_session_open(context: ContextTypes.DEFAULT_TYPE):
    name = context.job.name if context.job else "session"
    if name == "london_open":
        text = "✅ *Session Open*\n🌍 London Open — 08:00 UTC (09:00 WAT)"
    elif name == "ny_open":
        text = "✅ *Session Open*\n🗽 NY Open — 13:00 UTC (14:00 WAT)"
...
async def send_weekly_review_prompt(context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "🔥 Best model: [N/A] ([0])\n"
        "🕐 Scheduled review time: 21:00 WAT (20:00 UTC Sunday)\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def send_end_of_day_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    wat_date = _now_wat().date()
    txt = (
        f"🌙 End of Day Summary — [{wat_date}]\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📡 Setups fired:    [{setups}]\n"
        f"✅ Trades taken:    [{trades}]\n"
        f"💰 R today:         [{r:+.2f}]\n"
        f"🛡️ Discipline:      [{score}]/100\n"
        f"📉 Loss streak:     [{streak}]\n"
        "🕐 Report time:     [22:00 WAT]\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{mot}"
    )
...
async def send_news_pre_warning(context: ContextTypes.DEFAULT_TYPE):
    await context.application.bot.send_message(chat_id=CHAT_ID, text="📰 News Warning — NFP\nPair: [BTCUSDT] | Time: [13:00 WAT] | Impact: 🔴 High\n⚠️ Alerts for [BTCUSDT] will be suppressed from [12:30 WAT] to [13:15 WAT]", parse_mode='Markdown')
