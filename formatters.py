...
from config import VIOLATION_LABELS
...
def fmt_home(active_models, live_alerts):
    return f"📊 *Mission Control Dashboard*\n━━━━━━━━━━━━━━━━━━━━━━━━\nActive models: `{len(active_models)}`\nRecent setups: `{len(live_alerts)}`"
...
def fmt_status(session, db_ok, active_models, prices_ok): return f"⚡ *System Status*\nSession `{session}`\nDB {'OK' if db_ok else 'ERR'}"
def fmt_backtest(model,result,days): return f"🧪 *Backtest* {model['name']} {days}d"

def fmt_stats_overview(row):
    total=row.get('total') or 0; wins=row.get('wins') or 0; wr=round((wins/total)*100,1) if total else 0
...
def fmt_rolling_10(trades):
    ...
    wins=sum(1 for t in trades if t.get('result')=='TP'); total=len(trades)
    wr=round((wins/total)*100,1) if total else 0
    total_r=round(sum(float(t.get('rr') or 0) for t in trades),2)
    first=sum(float(t.get('rr') or 0) for t in trades[:5]); last=sum(float(t.get('rr') or 0) for t in trades[5:])
...
def fmt_heatmap(hourly_data):
    rows={int(x['hour']):x for x in hourly_data}; scored=[]
    for h in range(24):
        d=rows.get(h,{"wins":0,"total":0,"total_r":0}); t=int(d.get('total') or 0); wr=round((int(d.get('wins') or 0)/t)*100,1) if t else 0; r=float(d.get('total_r') or 0); scored.append((h,wr,r))
    top={x[0] for x in sorted(scored,key=lambda x:x[2],reverse=True)[:3]}; bot={x[0] for x in sorted(scored,key=lambda x:x[2])[:3]}
